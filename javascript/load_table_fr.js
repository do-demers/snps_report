...
dispatch.on("load_table", function (tbl_data, resp_table) {
    ...
    var dept_en = _.uniq(_.pluck(_.where(resp_table, {dept_fr: start_dept[0]}),"dept_en"))

    d3.select("#pdf_div")
        // .attr("href", "pdf/" + dept_en + "_fr.pdf")
        .attr("href", "pdf/" + dept_en[0].toLowerCase().replace(/\s/g, '-') + "_fr.pdf")

    d3.select("#org_th").text(_.uniq(_.pluck(_.where(resp_table, {dept_fr: start_dept[0]}),"dept_fr")));
    d3.select("#tbl_emp").text(_.pluck(_.where(resp_table, {dept_fr: start_dept[0], respondent: "Number of employees" }),"n" ));
    d3.select("#tbl_resp").text(_.pluck(_.where(resp_table, {dept_fr: start_dept[0], respondent: "Number of respondents" }),"n" ));
    d3.select("#tbl_stf").text(_.pluck(_.where(resp_table, {dept_fr: start_dept[0], respondent: "Number of staffing advisors" }),"n" ));
    d3.select("#tbl_man").text(_.pluck(_.where(resp_table, {dept_fr: start_dept[0], respondent: "Number of managers or supervisors" }),"n" ));
    d3.select("#tbl_man_2").text(_.pluck(_.where(resp_table, {dept_fr: start_dept[0], respondent: "Number of managers who hired or attempted to hire an indeterminate or term employee between January 1 and December 31, 2017" }),"n"));
    ...
    var filt_SOS_data = _.filter(tbl_data, function (row) {
        return (_.contains(start_dept, row.DEPT)  || _.contains(dept_size, row.DEPT) || _.contains(["Toutes les organisations"], row.DEPT)) && _.contains(start_Q, row.Question);
    });
    ...
    dispatch.on("update_table", function (d) {
        ...
        new_tds.enter().append('td')        .attr("scope",function (d,i) {
            return i === 0 ? "row" : undefined;
        }).html(function (d) {
                return isNaN(d.value) ? d.value : d.column === "total" ? d.value : fmt_pct(d.value);
        });
